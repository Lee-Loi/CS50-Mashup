...
function addMarker(place)
{
    ...
    google.maps.event.addListener(marker, "click", function() { articles(marker, place["postal_code"] + this);
    
            // retrieve weather
    $.getJSON("http://api.openweathermap.org/data/2.5/weather?zip=" + place["postal_code"] + ",us&mode=json&appid=2fe7793327020730d857b42d21d419cd",function(result){

    // loads in weather icon
...
function weatherdata(marker, query) 
{
 
    $.getJSON("http://api.openweathermap.org/data/2.5/weather?zip=" + query + ",us&mode=json&appid=2fe7793327020730d857b42d21d419cd")
    .done(function(data, textStatus, jqXHR) {
    ...
    // loads in weather icon
    weatherInfo = ("<p class='weather'>"+data.weather[0].description+"</p>");
    var link = ("http://openweathermap.org/img/w/" + data.weather[0].icon + ".png");
    weatherInfo += ("<img class='weather' src=" + (link) + ">");
...
function articles(marker, query, weatherInfo) 
{
    ...
    $.getJSON("articles.php?geo=" + query)
    .done(function(data, textStatus, jqXHR) {
    ...
    showInfo(marker,html + weatherInfo); 

    //showInfo(marker,weatherInfo); 
